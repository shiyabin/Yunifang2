...
public class LogDog {
    public static final SimpleDateFormat DATA_FORMAT_TIME = new SimpleDateFormat("MM-dd HH:mm:ss.SSS");
    ...
    private static String getLogFilePath(boolean creatIfNotExist) {
        ...
        File appFile = new File(Environment.getExternalStorageDirectory() + "/BilinLog/appLog.txt");
        if (!appFile.exists()) {
            try {
                if (creatIfNotExist) {
